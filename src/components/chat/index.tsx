...
export default function AIChat({
    ...
    const { messages, isLoading, isAnyLoading, input, handleInputChange, handleSubmit, handleStop, sendMessage } = useAIChat({
    ...
    useEffect(() => {
        if (messageGroups.length > 0) {
            ...
            if (lastGroup.assistantMessage && !collapsedGroups.has(lastIndex) && !manuallyToggledGroups.has(lastIndex)) {
                setCollapsedGroups(prev => {
                    const newSet = new Set(prev);
        ...
        const loadPromptSuggestions = async (abortSignal: AbortSignal) => {
            ...
            const hasLoadingMessage = messages.some(
                msg =>
                    msg.role === 'assistant' &&
                    Array.isArray(msg.content) &&
                    msg.content.some(block => block.type === 'text' && block.text.includes('を分析中... おすすめの分析を生成しています...'))
            );
                ...
                if (abortSignal.aborted) {
                    ...
                    const cleanMessages = messages.filter(msg => {
                        if (msg.role === 'assistant' && Array.isArray(msg.content)) {
                            return !msg.content.some(block => block.type === 'text' && block.text.includes('を分析中... おすすめの分析を生成しています...'));
                        }
                        return true;
                    });
    ...
    return (
        <div className="p-2.5 bg-gray-100 text-gray-800 text-left h-screen flex flex-col overflow-hidden">
            <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto bg-white border border-gray-300 rounded-md p-2.5 mb-2.5">
                {messages.length === 0 && <p className="text-gray-500">チャットを開始しましょう。データの可視化やモデリングについて質問してみてください。</p>}
