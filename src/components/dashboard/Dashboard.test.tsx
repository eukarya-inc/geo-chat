  ...
  describe('Chart Configuration Extraction', () => {
    it('should extract configuration from scatter plot chart spec', () => {
      ...
      const extractConfig = (spec: ChartSpec) => {
        ...
        if ('mark' in spec.spec && typeof spec.spec.mark === 'object' && spec.spec.mark !== null && 'type' in spec.spec.mark) {
      ...
      const extractConfig = (spec: ChartSpec) => {
        ...
        if ('mark' in spec.spec && typeof spec.spec.mark === 'object' && spec.spec.mark !== null && 'type' in spec.spec.mark) {
          if (spec.spec.mark.type === 'bar') {
            ...
            if ('encoding' in spec.spec && spec.spec.encoding && 'x' in spec.spec.encoding &&
                spec.spec.encoding.x && typeof spec.spec.encoding.x === 'object' &&
                'bin' in spec.spec.encoding.x && spec.spec.encoding.x.bin) {
              plotType = 'histogram';
            }
          }
        }
