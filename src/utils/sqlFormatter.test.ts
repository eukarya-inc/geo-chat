...
describe('SQL Formatter', () => {
  describe('formatSQL', () => {
    ...
    it('should format complex CREATE TABLE with CTE', () => {
      const sql = 'WITH yearly_data AS (SELECT 事業者名, unnest.地域名, 事業概要.年度 FROM business_data, UNNEST(輸送実績) as unnest) CREATE TABLE business_metrics AS SELECT * FROM yearly_data';
      const formatted = formatSQL(sql);
  ...
  describe('formatSQLCompact', () => {
    ...
    it('should format complex queries with multiple lines', () => {
      const sql = 'WITH yearly_data AS (SELECT 事業者名 FROM business_data) CREATE TABLE metrics AS SELECT * FROM yearly_data';
      const formatted = formatSQLCompact(sql);
      expect(formatted.split('\n').length).toBeGreaterThan(1);
    });

    it('should format queries longer than 100 chars', () => {
      const sql = 'SELECT column1, column2, column3, column4, column5, column6, column7, column8, column9, column10 FROM very_long_table_name';
      const formatted = formatSQLCompact(sql);
      expect(formatted.split('\n').length).toBeGreaterThan(1);
    });
  });
