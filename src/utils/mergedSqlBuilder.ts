...
export function buildMergedSql(
  createTableSqls: string[]
): { mergedSql: string; order: string[] } {
  // Map tableName -> SQL
...
export function buildSingleCreateSqlForTarget(
  targetTable: string,
  createTableSqls: string[]
): string {
  const tableToSQL = new Map<string, string>();
  ...
  const reachableSqls: string[] = depNames
  ...
  function rewriteSelect(sql: string): string {
    ...
    for (const [orig, alias] of cteNames.entries()) {
      const pattern = new RegExp(
        `(^|[^\\w.])(${escapeRegex(orig)}|"${escapeRegex(orig)}"|\`${escapeRegex(orig)}\`)(?=$|[^\\w.])`,
        'g'
      );
