  ...
  describe('fixMaplibreExpression', () => {
    describe('get expressions', () => {
      ...
      it('should handle array indexing attempt (common AI mistake)', () => {
        ...
        expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot access array element 輸送実績[0].営業収入_千円 in MapLibre expressions. Using fallback value null instead.');
      });
      ...
      it('should remove invalid default value from simple get', () => {
        ...
        expect(fixMaplibreExpression(expr)).toEqual(expected);
        expect(consoleWarnSpy).toHaveBeenCalledWith('Removed invalid default value 100 from ["get", "property", 100]. Use ["coalesce", ["get", "property"], 100] if you need a default value.');
      });
      ...
      it('should fix multiple levels of array access attempts', () => {
        ...
        expect(fixMaplibreExpression(expr)).toEqual(expected);
        expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot access array element level3[999].level2 in MapLibre expressions. Using fallback value null instead.');
      });
    ...
    describe('case expressions', () => {
      ...
      it('should fix array indexing attempts within case conditions', () => {
        const expr = [
          'case',
          ['>=', ['get', '営業収入_千円', ['get', '輸送実績', 0]], 10000], '#ff4444',
          '#aaaaaa'
        ];
        const expected = [
          'case',
          ['>=', ['literal', null], 10000], '#ff4444',
          '#aaaaaa'
        ];
        expect(fixMaplibreExpression(expr)).toEqual(expected);
        expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot access array element 輸送実績[0].営業収入_千円 in MapLibre expressions. Using fallback value null instead.');
    ...
    describe('interpolate expressions', () => {
      ...
      it('should handle array access attempt in interpolate', () => {
        ...
        expect(fixMaplibreExpression(expr)).toEqual(expected);
        expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot access array element metrics[0].revenue in MapLibre expressions. Using fallback value null instead.');
      });
    });
    ...
    describe('at expressions', () => {
      it('should detect and fix invalid property access using at expression', () => {
        ...
        const expected = ['literal', null];
        expect(fixMaplibreExpression(expr)).toEqual(expected);
        expect(consoleWarnSpy).toHaveBeenCalledWith('Cannot access property "営業収入_千円" of array element 輸送実績[10] in MapLibre expressions. Arrays of objects are not supported for property styling. Consider flattening your data structure in SQL.');
      });

      it('should fix at expression in case statement', () => {
        const expr = [
          'case',
          ['>=', ['get', '営業収入_千円', ['at', 10, ['get', '輸送実績']]], 10000],
          '#ff4444',
          '#aaaaaa'
        ];
        const expected = [
    ...
    describe('fixMaplibreExpressionWithWarnings', () => {
      it('should return warnings array with fixed expression', () => {
        ...
        expect(result.warnings[0]).toBe('Removed invalid default value 100 from ["get", "property", 100]. Use ["coalesce", ["get", "property"], 100] if you need a default value.');
      });
