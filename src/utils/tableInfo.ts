...
export async function getTableInfo(dbContext: DBContext, tableName: string, schema: string | null): Promise<TableInfo> {
    ...
    try {
        ...
        // Get row count
        const countQuery = schema ? `SELECT COUNT(*) as count FROM ${schema}.${tableName}` : `SELECT COUNT(*) as count FROM ${tableName}`;
        ...
        try {
            ...
            for (const column of schemaData) {
                ...
                if (!column.column_type.toUpperCase().includes('GEOMETRY') && !isComplexType(column.column_type)) {
                    try {
                        const stats = await getColumnStatistics(dbContext, tableName, column.column_name, column.column_type, schema);
        ...
        if (info.columnStatistics) {
            ...
            if (dateColumns.length > 0 && info.suggestions) {
                ...
                dateColumns.slice(0, 1).forEach(({ name, stats }) => {
                    suggestions.push(`「${name}」列は時系列データ（${stats.minDate} 〜 ${stats.maxDate}）です。折れ線グラフでの時系列分析が適しています。`);
                });
            }
