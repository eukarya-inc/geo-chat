...
export async function generateDefaultCharts(tableName: string, dbContext: DBContext, schema: string | null = null): Promise<ChartGenerationResult[]> {
    try {
        ...
        // 4. If we have only numeric columns, create bar chart of first column
            ...
            const indexColumn = columns.find(
                col => col.name.toLowerCase().includes('index') || col.name.toLowerCase().includes('id') || col.name.toLowerCase() === 'rowid'
            );
...
function isCategoricalType(type: string): boolean {
    const lowerType = type.toLowerCase();
    return lowerType.includes('varchar') || lowerType.includes('text') || lowerType.includes('string') || lowerType.includes('char');
}
