...
function convertSpecialValues(value: unknown, columnType?: string): unknown {
    ...
    if (value instanceof Uint8Array || value instanceof ArrayBuffer || (value && typeof value === 'object' && 'byteLength' in value)) {
...
export async function getTableData(
    ...
    const [columns, totalRows] = await Promise.all([getTableSchema(connection, tableName), getTableRowCount(connection, tableName)]);
    ...
    // Use the table name as-is (it might already include schema prefix)
    const result = await connection.query(`SELECT ${columnNames} FROM ${tableName}${orderByClause} LIMIT ${limit} OFFSET ${offset}`);
...
export function getValueFromArrowTable(arrowTable: ArrowTable, rowIndex: number, columnIndex: number, columnType?: string): unknown {
 ...
 */
export async function checkTableGeometry(connection: AsyncDuckDBConnection, tableName: string): Promise<GeometryCheckResult> {
    try {
        ...
        const geometryColumns = columnInfo.filter(col => col.column_type && col.column_type.toUpperCase().includes('GEOMETRY')).map(col => col.column_name);
