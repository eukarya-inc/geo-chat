 ...
 */
export async function geocodeMultipleAddresses(addresses: string[], rateLimitMs: number = 1000): Promise<{ results: GeocodeResult[]; errors: GeocodeError[] }> {
...
export async function analyzeTableForGeocoding(
    ...
    for (const column of schema) {
        ...
        if (columnType.toLowerCase().includes('varchar') || columnType.toLowerCase().includes('text')) {
            // Get sample values
            const sampleResult = await dbContext.executeQuery(`SELECT DISTINCT ${columnName} FROM ${tableName} WHERE ${columnName} IS NOT NULL LIMIT 5;`, null);
