...
describe('DBContext Browser Integration', () => {
    ...
    describe.concurrent('getTableColumns', () => {
      ...
      `);
        ...
        it.concurrent('should throw error for non-existent table', async () => {
            await expect(dbContext.getTableColumns('non_existent_table')).rejects.toThrow("Table 'non_existent_table' does not exist or is not accessible");
        });
    });
    ...
    describe('executeWithRefresh', () => {
        it('should execute operation and notify listeners', async () => {
            ...
            await dbContext.executeWithRefresh(() => dbContext.executeQuery('CREATE TABLE refresh_test (id INTEGER)'), 'refresh_test');
        ...
        it('should validate table after creation', async () => {
            await dbContext.executeWithRefresh(() => dbContext.executeQuery('CREATE TABLE validated_table (id INTEGER)'), 'validated_table');
