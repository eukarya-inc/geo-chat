...
describe('tableUrlHelpers', () => {
    ...
    describe('getFromClauseForUrl', () => {
        ...
        it('should return read_csv_auto for CSV files', () => {
            expect(getFromClauseForUrl('https://example.com/data.csv')).toBe("read_csv_auto('https://example.com/data.csv')");
            expect(getFromClauseForUrl('https://example.com/data.CSV')).toBe("read_csv_auto('https://example.com/data.CSV')");
        });

        it('should return st_read for geospatial files', () => {
            expect(getFromClauseForUrl('https://example.com/data.geojson')).toBe("st_read('https://example.com/data.geojson')");
            expect(getFromClauseForUrl('https://example.com/data.shp')).toBe("st_read('https://example.com/data.shp')");
            expect(getFromClauseForUrl('https://example.com/data.json')).toBe("st_read('https://example.com/data.json')");
        });

        it('should default to st_read for unknown extensions', () => {
            expect(getFromClauseForUrl('https://example.com/data.unknown')).toBe("st_read('https://example.com/data.unknown')");
            expect(getFromClauseForUrl('https://example.com/data')).toBe("st_read('https://example.com/data')");
        });

        it('should handle URLs with query parameters', () => {
            expect(getFromClauseForUrl('https://example.com/data.csv?token=abc')).toBe("read_csv_auto('https://example.com/data.csv?token=abc')");
            expect(getFromClauseForUrl('https://example.com/data.parquet?version=2')).toBe("'https://example.com/data.parquet?version=2'");
        });
    });
});
