...
export function detectGeometryColumns(schema: Array<{column_name: string, column_type: string}>): string[] {
  return schema
    .filter(col => 
      col.column_type.toUpperCase() === 'GEOMETRY' ||
      col.column_type.toUpperCase().startsWith('GEOMETRY(')
    )
 ...
 */
export async function getGeometryTypes(
  dbContext: DBContext,
  tableName: string,
  columnName: string,
  schema: string | null
 ...
 */
export async function analyzeTableGeometry(
  dbContext: DBContext,
  tableName: string,
  schema: string | null
...
export function formatGeometryInfo(geometryInfo: GeometryInfo[]): string {
  return geometryInfo
    .map(info => `${info.columnName} (${info.geometryType})`)
    .join(', ');
}