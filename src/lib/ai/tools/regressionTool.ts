...
/**
 * Perform regression analysis on a DuckDB table
 * This function can be called directly from other tools
 */
export async function performRegressionAnalysis(
    ...
    const { table_name, target_column, explanatory_columns } = params;
    try {
        ...
        if (missingColumns.length > 0) {
            return errorResponse(`説明変数のデータが取得できませんでした（不足: ${missingColumns.join(', ')}）。`);
        }
...
3. Use layered chart spec with scatter points (layer 1) and regression line (layer 2)`,
        ...
        execute: async ({ table_name, target_column, explanatory_columns }): Promise<RegressionAnalysisResponse> => {
            return performRegressionAnalysis(dbContext, schema, {
                table_name,
                target_column,
                explanatory_columns,
            });
