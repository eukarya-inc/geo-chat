...
describe('duckdbTool AI invocation (browser, real DuckDB-WASM)', () => {
    ...
    it('returns all rows from SELECT and truncates for AI response', async () => {
        ...
        const createRes = await tool.execute({ sql: 'CREATE TABLE nums AS SELECT range as id FROM range(0, 150)' }, { messages: [], toolCallId: '' });
        ...
        const selectRes = await tool.execute({ sql: 'SELECT id FROM nums ORDER BY id' }, { messages: [], toolCallId: '' });
    ...
    it('creates a table with geometry and detects geometry info', async () => {
        ...
        const res = await tool.execute({ sql: 'CREATE TABLE test_points AS SELECT ST_Point(139.7, 35.6) as geom, 1 as id' }, { messages: [], toolCallId: '' });
    ...
    it('handles CREATE OR REPLACE TABLE with geometry', async () => {
        ...
        await tool.execute({ sql: 'CREATE TABLE IF NOT EXISTS test_replace AS SELECT 1 as id' }, { messages: [], toolCallId: '' });
    ...
    it('does not return geometry info for SELECT queries', async () => {
        ...
        await tool.execute({ sql: 'CREATE TABLE select_test_geom AS SELECT ST_Point(139.7, 35.6) as geom' }, { messages: [], toolCallId: '' });
    ...
    it('truncates large result sets for AI response', async () => {
        ...
        await tool.execute({ sql: 'CREATE TABLE large_table AS SELECT range as id FROM range(0, 1500)' }, { messages: [], toolCallId: '' });
    ...
    it('generates SQL explanation when API key is provided', async () => {
        ...
        const res = await tool.execute({ sql: 'CREATE TABLE test_explain AS SELECT 1 as id' }, { messages: [], toolCallId: '' });
