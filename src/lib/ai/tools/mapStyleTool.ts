                        ...
                        if (schemaResult && schemaResult.length > 0) {
                            ...
                            if (!hasGeometry) {
                                const columnInfo = schemaResult
                                    .map((row: Record<string, unknown>) => `${row.column_name || row.name} (${row.column_type || row.type})`)
                                    .join(', ');
...
Available columns in '${table_name}': ${columnInfo}`,
                ...
                const mapLibreLayerType = geometry_type === 'point' ? 'circle' : geometry_type === 'line' ? 'line' : 'fill';
                        ...
                        try {
                            ...
                            if (layerErrors && layerErrors.length > 0) {
                                ...
                                layerErrors.forEach(error => {
                                    const errorMessage = error.message.toLowerCase();
                                    if (errorMessage.includes('unknown') || errorMessage.includes('invalid') || errorMessage.includes('required')) {
