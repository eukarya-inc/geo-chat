...
import {
  ...
  getGeometryTypes,
  analyzeTableGeometry,
  formatGeometryInfo
} from './geometryDetector';
...
describe('geometryDetector', () => {
  ...
  describe('getGeometryTypes', () => {
    it('should get geometry types using ST_GeometryType', async () => {
      ...
      const result = await getGeometryTypes(
        mockDbContext as unknown as DBContext,
        'test_table',
        'geom',
        null
      );
  ...
  describe('analyzeTableGeometry', () => {
    it('should analyze table with geometry columns', async () => {
      const mockDbContext = {
        executeQuery: vi.fn()
          .mockResolvedValueOnce([ // DESCRIBE result
            { column_name: 'id', column_type: 'INTEGER' },
            { column_name: 'geom', column_type: 'GEOMETRY' }
          ])
          .mockResolvedValueOnce([ // ST_GeometryType result
            { geom_type: 'POINT' }
          ])
      };
      
      const result = await analyzeTableGeometry(
        mockDbContext as unknown as DBContext,
        'test_table',
        null
      );
      ...
      expect(result).toEqual({
        hasGeometry: true,
        geometryInfo: [
          { columnName: 'geom', geometryType: 'POINT' }
        ]
      });
    });
    ...
    it('should fallback to column type when ST_GeometryType fails', async () => {
      ...
      expect(result).toEqual({
        hasGeometry: true,
        geometryInfo: [
          { columnName: 'geom', geometryType: 'GEOMETRY(POLYGON)' }
        ]
      });
    });
