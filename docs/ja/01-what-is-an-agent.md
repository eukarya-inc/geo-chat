...
> このワークショップの最初の 20 分。まず「魔法」を見て、次にそれをわざと壊して、
...
> **試してみる: エージェントは「知らないこと」を正直に言う**
> 続けて「人口 10 万人以上の市を塗り分けて」と頼んでみてください。組み込みデータ
> `japan_cities` には人口カラムがありません。エージェントはスキーマを調べたうえで
> 「人口データが無い」と **正直に答え、何も塗りません**（それらしい数字を **幻覚しない**）。
> これは、エージェントが思い込みで動くのではなく **実際にスキーマを見て行動している** 証拠です。
> 人口データを別途結合すれば実現できます（[07. 実践課題](./07-challenge.md) 参照）。
...
- **この教材の中で**: 後述の壊す実験 #1（§③）でツールを全部外すと、エージェントが
