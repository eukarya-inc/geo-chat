...
5. あるリクエストの `system` 全文をコピーし、そのコピーの中に `ROLE_AND_ENV`（`## How to work` の前にある、見出しの付いていないブロック） / `## How to work` / `## Built-in datasets` / `## Rules`（tier ごとに静的）と、末尾の `## Context`（毎ターン動的）の境目に印を付けます。テーブルをもう 1 つ読み込んで同じ質問をもう一度し、末尾だけが変わる様子を観察します。
